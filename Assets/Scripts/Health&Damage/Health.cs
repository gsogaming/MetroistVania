...
public class Health : MonoBehaviour
{
    ...
    [Tooltip("Whether this is hurt or not")]
    public bool isHurt = false;



    [Header("DamageIndicatorSettings")]
    [Tooltip("The overlay effect to show that the player is damaged")]
    public Image damageIndicator;

    [Tooltip("The duration of the fade-out effect for the damage indicator")]
    public float fadeDuration = 1f;

    private Coroutine fadeCoroutine; // Stores a reference to the fading coroutine
    private float currentAlpha = 0f; // The current alpha value of the damage indicator
    ...
    void Respawn()
    {
        ...
        {
            // Reset the damage indicator to zero alpha when the player respawns
            damageIndicator.color = new Color(damageIndicator.color.r, damageIndicator.color.g, damageIndicator.color.b, 0f);
        }
    ...
    public void TakeDamage(int damageAmount)
    {
        ...
        else
        {
            if (this.gameObject.tag == "Player")
            {
                ...
                // Set the alpha to 1 to make the damage indicator fully visible
                currentAlpha = 1f;
                damageIndicator.color = new Color(damageIndicator.color.r, damageIndicator.color.g, damageIndicator.color.b, currentAlpha);

                // Start the fading coroutine
                fadeCoroutine = StartCoroutine(FadeOutDamageIndicator());
            }



            if (hitEffectSFX != null)
            {
                Instantiate(hitEffectSFX, transform.position, transform.rotation, null);
            }
    ...
    private IEnumerator FadeOutDamageIndicator()
    {
        ...
        while (elapsedTime < fadeDuration)
        {
            ...
            currentAlpha = Mathf.Lerp(startAlpha, 0f, elapsedTime / fadeDuration);
            damageIndicator.color = new Color(damageIndicator.color.r, damageIndicator.color.g, damageIndicator.color.b, currentAlpha);

            yield return null;
        }

        // Ensure the alpha value is set to 0 when the fading is finished
        currentAlpha = 0f;
        damageIndicator.color = new Color(damageIndicator.color.r, damageIndicator.color.g, damageIndicator.color.b, currentAlpha);
    }
    ...
    public GameObject hitEffectSFX;
    ...
    void Die()
    {
        ...
        if (this.gameObject.tag == "Player")
        {
            // Set the damage indicator to full alpha when the player dies
            currentAlpha = 1f;
            damageIndicator.color = new Color(damageIndicator.color.r, damageIndicator.color.g, damageIndicator.color.b, currentAlpha);
